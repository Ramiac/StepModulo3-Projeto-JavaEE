...
<head>
    <meta charset="UTF-8">
    <title>InstaFood - restaurante: <%=request.getAttribute("nome")%></title>
...
<header class="p-3 mb-3 border-bottom">
    ...
    <main>
        <%
            boolean reservar = (boolean) request.getAttribute("reservar");
            boolean retirada = (boolean) request.getAttribute("retirada");
            boolean rappi = (boolean) request.getAttribute("rappi");
            boolean iFood = (boolean) request.getAttribute("iFood");
            boolean uberEats = (boolean) request.getAttribute("uberEats");
        %>
        <div class="container">
        <h3>Pagina do Restaurante: <%=request.getAttribute("nome")%></h3>
        <div class="row justify-content-md-center">
            <div class="col-lg-6">
            <p> <span class="fw-bold">Nome: </span><%=request.getAttribute("nome")%></p>
            <p><span class="fw-bold">Descrição: </span> <%=request.getAttribute("descricao")%> </p>
            <p><span class="fw-bold">CNPJ: </span><%=request.getAttribute("cnpj")%></p>
            <p><span class="fw-bold">eMail: </span><%=request.getAttribute("email")%></p>
            <p><span class="fw-bold">Telefone: </span><%=request.getAttribute("telefone")%></p>
            <p class="fw-bold">Opção de compra: </p>
            <ul>
                
                <% 
                    if (reservar) out.print("<li>Reservação por telefone ou por email.</li>"); 
                    if (retirada) out.print("<li>Retirada no Endereço.</li>"); 
                    if (rappi) out.print("<li>Delivery pelo <a href='https://www.rappi.com.br' target='_blank' rel='noopener noreferrer'>Rappi</a></li>"); 
                    if (iFood) out.print("<li>Delivery pelo <a href='https://www.ifood.com.br/' target='_blank' rel='noopener noreferrer'>iFood</a></li>"); 
                    if (uberEats) out.print("<li>Delivery pelo <a href='https://www.ubereats.com/br' target='_blank' rel='noopener noreferrer'>UberEats</a></li>"); 
                %>
            </ul>
            <span>Foto principal restaurante: </span><img src="img/restaurante.jpg" width="100px" height="100px">
            <br>
            </div>
            <div class="col-lg-10">
            <p class="fw-bold">Endereços: </p>
            <%
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <% 
                            ...
                            out.println(
                                "Esse restaurante não tem nenhum endereço cadastrado ainda."
                            );
                        } 
                    %>
                </tbody>
